...
public class Base4_Synchronized implements Runnable {
    ...
    static class InsertData {
        public synchronized void insert1(String name) {
        ...
        public synchronized static void insert2(String name) {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        new Thread() {
            @Override
            public void run() {
                insertData.insert1("A"); // 对象锁
            }
        }.start();
        new Thread() {
            @Override
            public void run() {
                insertData.insert2("B"); // 类锁
            }
        }.start();
        new Thread() {
            @Override
            public void run() {
                insertData.insert2("C"); // 类锁
            }
        }.start();
