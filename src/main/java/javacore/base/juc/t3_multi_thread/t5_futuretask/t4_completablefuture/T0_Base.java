...
public class T0_Base {
    ...
    public static void main(String[] args) {
        ...
        // -supply: 无输入,有输出
        CompletableFuture<Object> future = FutureUtil.getFuture("A", 2000, 5);

        Thread.sleep(5000);
        System.out.println("睡眠完成——验证 CompletableFuture 在创建的时候就开始执行了.");
        /*
         * future.join(): 阻塞的等待(不抛出异常,除非是unchecked)
         * future.get(): 阻塞的等待(有异常则抛出)
         * future.get(1,TimeUnit.Hours): 有异常则抛出异常，最长等待1个小时，一个小时之后，如果还没有数据，则异常。
         */
        Object o = future.get();
        System.out.println("future 返回值:" + o);

        // 无输入,无输出
        thenRunAsync(FutureUtil.getFuture("A1", 2000, 5));
        // 有输入,无输出
        thenAcceptAsync(FutureUtil.getFuture("A2", 2000, 5));
        // 有输入, 有输出
        thenApplyAsync(FutureUtil.getFuture("A3", 2000, 5));
